...
const DropdownNotification = () => {
  ...
  const fetchNotifications = async () => {
    try {
      const response = await httpGet(`notifications?type=order`);
  ...
  return (
    <ClickOutside onClick={() => setDropdownOpen(false)} className="relative">
      <li>
        ...
        >
          {notificationCount > 0 && (
            <span className="absolute -right-1 -top-1 flex h-5 w-5 items-center justify-center rounded-full bg-red-600 text-xs font-semibold text-white shadow-md">
              {notificationCount}
            </span>
          )}
